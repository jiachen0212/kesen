...
def merge_heixian(xs, ys, hs, ws, mean_scores, hx_areas, gray_values, x_dis=20, y_dis=150):
    # 先把x从小到大排序, 得到index顺序
    lens = len(xs)
    ...
    hx_areas_ = [hx_areas[k] for k in sorted_id]
    ...
    for r, merged_index_list in enumerate(all_merged):
        ...
        merged_box_grayvalue_score_area[3][r] = sum([hx_areas_[p] for p in merged_index_list])

    return merged_box_grayvalue_score_area


def sdk_post(heixian_index, heixianban, diangui_index, diangui_area_distance, predict_index, prdect_score_map, defects_lists, infer_image, x_dis, y_dis, Confidence=None, num_thres=None, apple_logo_mask=None):
    ...
    for cls in range(1, num_class):
        ...
        if defects_lists[cls] not in predict_result:
            predict_result[defects_lists[cls]] = [[],[],[],[]]   

        if cls == heixian_index:
            cur_defect = defects_lists[cls]
            heixian_mask = np.array(predict_index == cls, np.uint8)
            cc_output = cv2.connectedComponentsWithStats(heixian_mask, 8)
            ...
            for label in range(1, num_contours):
                # dl模型的heixian
                temp_predict += heixian_mask * label
                ...
                merged_box_grayvalue_score_area = merge_heixian(xs, ys, hs, ws, mean_scores, hx_areas, gray_values, x_dis=x_dis, y_dis=y_dis)
...
if __name__ == "__main__":
    ...
    sdk_post(heixian_index, heixianban, diangui_index, diangui_area_distance, full_index_predict, full_score_predict, defects, full_img,  hx_x_dis, hx_y_dis, Confidence=None, num_thres=None, apple_logo_mask=None):
