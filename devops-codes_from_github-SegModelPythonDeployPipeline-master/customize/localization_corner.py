...
def localize_for_corner(source_image, thre=None, expend=0):
    top, bottom = localize_corner_edge(source_image, find_in_vertical=True, thre=thre, expend=expend)
...
if __name__ == "__main__":
    ...
    # # 侧面
    dir_image = r"D:\Work\projects\kersen\data\side\DDS-R角"
    dir_out = r"D:\Work\projects\kersen\data\side\debug"
    func = localize_for_corner
    ...
    for root, dirs, files in os.walk(dir_image):
        for file in files:
            if is_image(file):
                ...
                left, top, right, bottom = func(source_image, expend=50)
